...
class ModelSceneController implements SetupSceneController {
	title = t("setup.model_title", "Choose your default model");
	subtitle = t("setup.model_subtitle", "Search configured models and save the model used for new sessions.");
	...
	async onMount(): Promise<void> {
		this.#status = theme.fg("muted", t("setup.discovering_models", "Discovering available models…"));
	...
	render(width: number, maxLines?: number): readonly string[] {
		const lines = [
			this.#status ?? theme.fg("muted", t("setup.model_search_hint", "Type to search. Enter saves the highlighted model as your default.")),
			"",
		];
	...
	async #select(model: Model, selector: string): Promise<void> {
		if (this.#selecting) return;
		this.#selecting = true;
		this.#status = theme.fg("muted", `${t("setup.saving_default_model", "Saving default model…")} (${selector})`);
		this.host.requestRender();
		try {
